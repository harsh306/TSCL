...
import numpy as np
from addition_rnn_curriculum import gen_curriculum_baseline, gen_curriculum_naive, gen_curriculum_mixed, \
    gen_curriculum_combined
from addition_rnn_model import AdditionRNNModel, AdditionRNNEnvironment
from tensorboard_utils_tf2 import create_summary_writer, add_summary
...
class WindowedSlopeBanditTeacher:
    ...
    def teach(self, num_timesteps=2000):
        for t in range(num_timesteps):
            slopes = [estimate_slope(timesteps, scores) if len(scores) > 1 else 1 for timesteps, scores in
...
if __name__ == '__main__':
    ...
    parser.add_argument('--teacher', choices=['curriculum', 'naive', 'online', 'window', 'sampling'],
                        default='sampling')
