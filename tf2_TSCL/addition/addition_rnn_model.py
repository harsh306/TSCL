...
class AdditionRNNModel(object):
    def __init__(self, max_digits=15, hidden_size=128, batch_size=4096, invert=True, optimizer_lr=0.001, clipnorm=None,
                 logdir=None):
        ...
        self.make_model()
        if logdir:
            self.callbacks = [tf.keras.callbacks.TensorBoard(log_dir=self.logdir)]
        else:
            self.callbacks = []

    def make_model(self):
        input = tf.keras.Input(shape=(self.maxlen, self.num_chars))

        # "Encode" the input sequence using an RNN, producing an output of HIDDEN_SIZE
        # note: in a situation where your input sequences have a variable length,
        # use input_shape=(None, nb_feature).
        x = tf.keras.layers.LSTM(self.hidden_size)(input)
        # For the decoder's input, we repeat the encoded input for each time step
        x = tf.keras.layers.RepeatVector(self.max_digits + 1)(x)
        # The decoder RNN could be multiple layers stacked or a single layer
        x = tf.keras.layers.LSTM(self.hidden_size, return_sequences=True)(x)
        # For each of step of the output sequence, decide which character should be chosen
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(self.num_chars, activation='softmax'))(x)
        ...
        self.model.compile(
            ...
            optimizer=tf.keras.optimizers.Adam(lr=self.optimizer_lr, clipnorm=self.clipnorm),
